...
const ClassModal = ({ isOpen, onClose, mode, classData, onSubmit, department, userRole, instituteId, teachers }) => {
  ...
  const fetchStudents = useCallback(async () => {
    ...
    setIsFetchingStudents(true);
    const classId = previousClassId || classData?._id
    if (classData || previousClassId) {
      console.log(classId);
      console.log(classData);
      console.log(previousClassId);
      
      
      try {

        const response = await axios.get(`/api/v2/utils/class-students?classId=${classId}&department=${formData.department}&acadmicYear=${formData.academicYear}`);
        if (response.data) {
          console.log(response.data);
          const students = response.data;
          setAllStudents(students);
          // Set selected students based on mode
          if (formData._id || previousClassId) {
          ...
          // Update batches with valid student IDs only
  ...
  }, [formData.academicYear, formData.department, classData, previousClassId]);
