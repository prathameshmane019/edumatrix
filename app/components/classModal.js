...
// import { getAcademicYears, getCurrentAcademicYear } from "./profile";
        ...
        <ModalBody>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <Select
              ...
              name="teacher"
              selectedKeys={[formData.teacher]}
              onSelectionChange={(value) => handleSelectChange("teacher", value.currentKey)}
              variant="bordered"
              size="sm"
            >
              {/* Assume teachers are fetched and passed as a prop */}
              {teachers.map((teacher) => (
                <SelectItem key={teacher._id} textValue={teacher.name}>
                  {teacher.name}
                </SelectItem>
