        ...
        <form onSubmit={handleSubmit}>
            <ModalBody>
                <Input
                    ...
                    name="index" // Make sure this is "index"
                    type="number"
                    min="1"
                    step="1"
...
export default function ManageProgramOutcomesPage() {
    ...
    const departmentScope = isAdmin && user?.id ? user.id : null;
    ...
    const fetchOutcomes = useCallback(async () => {
        console.log(selectedDept, selectedYear, user?.institute?._id || user?._id);
        ...
        try {
            ...
            if (response.data?._id) {
                ...
                console.log(documentId, response.data.data.programOutcomes, response.data.data.programSpecificOutcomes);
    ...
    const handleSave = async (formData) => {
        ...
        try {
            if (currentOutcome) {
                // Update existing outcome (assuming your PUT request handles updates correctly)
                const response = await axios.put(`/api/v2/obe/program-outcomes/${documentId}`, {
                    type: activeOutcomeType,
                    index: formData.index,
                    description: formData.description,
                    itemId: currentOutcome.index
                });
                ...
                // Create new outcome - Include all required fields here
                const response = await axios.post('/api/v2/obe/program-outcomes', {
                    institute: user.institute?._id || user?._id,
                    department: selectedDept,
                    academicYear: selectedYear,
                    type: activeOutcomeType, // Ensure this is being sent
                    index: formData.index,
                    description: formData.description
                });
    ...
    const handleDelete = async (itemId) => {
        ...
        console.log(documentId, itemId, activeOutcomeType);
