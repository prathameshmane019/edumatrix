  ...
  try {
    ...
    if (!institute || !department) {
      return NextResponse.json(
        { error: "Institute ID and Department are required" }, 
        { status: 400 }
      );
    }
    ...
    const classes = await Classes.find(
      filter,
      '_id id year batches' // Including batches in the projection
    );

    if (!classes || classes.length === 0) {
      return NextResponse.json(
        { message: "No classes found for this institute" }, 
        { status: 404 }
      );
    }

    const dropdownData = classes.map(cls => ({
      value: cls._id.toString(), // Ensure _id is converted to string
      label: `${cls.year} - ${cls.id}`,
      batches: cls.batches || [] // Ensure batches are included and defaulted to empty array
    }));
